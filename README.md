...
| ファイル | 内容 |
|---|---|
| `settings.json` | 設定（ホームフォルダ・サムネイルの大きさ・フォルダジャンプ・リサイズと連結で前回使った設定） |
...
- [x] 更新の確認: 起動時に 1 回だけ GitHub の最新リリースを確認し、新しければステータスバーとヘルプメニューに表示（作業は止めない）。
      クリックで変更内容を見て「更新する / 後で / このバージョンは飛ばす」。更新はチェックサム（SHA-256）を確かめてから exe を入れ替えて再起動。
      ヘルプメニューで起動時の確認を止められる（「更新を確認」で手動でも確認できる）。開発用のビルドでは起動時に確認しない
...
- [ ] UI の作り直し（v1.0 に向けて）: ツールバー 1 本化・右側のインスペクタ・高さ固定のフッターに操作ボタン・ダークモード。設計は [docs/ui-concept.md](docs/ui-concept.md)
...
## 画像の編集（選択中の画像に対して。右クリックメニュー・「画像」メニューからも）
