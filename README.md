...
- [ ] UI の作り直し（v1.0 に向けて）。設計は [docs/ui-concept.md](docs/ui-concept.md)
  ...
  - [x] 配色: システムに合わせる / ライト / ダーク（☰ → 表示 → テーマ。フッター右端のボタンでライト ↔ ダーク）
  ...
  - [x] リサイズ ▾ / Ctrl+Shift+R: 前回の設定のまま実行（設定画面を出さない）。新しいファイルを作るだけのときに限り、
  ...
  - [x] アドレスバー: ふだんはパンくず（クリックでその階層へ。長いときは先頭を「…」に）、空いている所のクリック / Ctrl+L で入力。
        入力すると ☰ のコマンド（先に数件）とフォルダを一緒に出す。「>」で始めるとコマンドだけ（「>」だけで全部）。
        ひらがな / カタカナ・全角 / 半角は区別しない。今は実行できないコマンドは薄く出す
  - [x] ダイアログのダーク対応（リサイズ・切り抜き・連結・名前の変更・フォルダーへ移動 / コピー・更新 など。
        Windows の確認メッセージ・フォルダの選択・色の選択の画面は Windows のまま）
