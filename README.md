...
| メタデータ | EXIF・XMP・IPTC を消す（既定。色の情報 ICC は残す）。回転は反映してから消すので向きは崩れない |
| ファイル名 | 置換 → 末尾に付ける → 小文字化 |
| 出力先 | 中のフォルダ（既定 `resized`）/ 同じフォルダ / 指定のフォルダ。同名のファイルは飛ばす / 上書き |

- 実行前に「元の画像 → 出力」を一覧で確認できる。出力の名前が重なるもの（`a.jpg` と `a.png` を両方 PNG に など）はエラーとして示し、直すまで実行できない
...
- 設定を変えるとすぐプレビューに反映する。プレビューは縮小して読んだ画像で組み（長辺 1200px まで）、保存するときだけ原寸で読む
